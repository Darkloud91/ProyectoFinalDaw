...
class main_module
{
	...
	function main($id, $mode)
	{
		...
		*/
		...
		foreach ($display_vars['vars'] as $config_name => $null)
		{
			...
			if ($submit)
			{
                if($config_name == 'tapatalk_custom_replace')
                {
                    $phpbb_container->get('config_text')->set($config_name, $config_value);
	...
	function show_custom_replacement($value, $key)
    {
        ...
        $customReplace = $phpbb_container->get('config_text')->get('tapatalk_custom_replace');
        if(!empty($config['tapatalk_custom_replace']) && empty($customReplace))
        {
            $customReplace = $config['tapatalk_custom_replace'];
        }
