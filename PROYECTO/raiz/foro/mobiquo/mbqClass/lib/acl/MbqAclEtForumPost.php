...
Class MbqAclEtForumPost extends MbqBaseAclEtForumPost {
    ...
    public function canAclReplyPost($oMbqEtForumPost) {
        global $user,$auth;
        if (!$user->data['is_registered'])  return $user->lang['LOGIN_EXPLAIN_POST'];
        ...
        if ($post_data['forum_password'] && !check_forum_password($forum_id))
             return $user->lang['LOGIN_FORUM'];

        // Check permissions
        if ($user->data['is_bot'])  return $user->lang['NOT_AUTHORISED'];
        ...
        if (!$auth->acl_get('f_read', $forum_id))
        {
            ...
            {
                 return $user->lang['USER_CANNOT_READ'];
            }
            
             return $user->lang['LOGIN_EXPLAIN_POST'];
        }
        ...
        {    
            if ($user->data['user_id'] != ANONYMOUS)
            {
                 return $user->lang['USER_CANNOT_REPLY'];
            }
        ...
        {
             return $user->lang['USER_CANNOT_FORUM_POST'];
        }

        // Forum/Topic locked?
        if (($post_data['forum_status'] == ITEM_LOCKED || (isset($post_data['topic_status']) && $post_data['topic_status'] == ITEM_LOCKED)) && !$auth->acl_get('m_edit', $forum_id))
        {
            return (($post_data['forum_status'] == ITEM_LOCKED) ? $user->lang['FORUM_LOCKED'] : $user->lang['TOPIC_LOCKED']);
        }
    ...
    public function canAclGetRawPost($oMbqEtForumPost) {
        ...
        {
            return $user->lang['USER_CANNOT_EDIT'];
        }
        ...
        {
            return (($topic_data['forum_status'] == ITEM_LOCKED) ? $user->lang['FORUM_LOCKED'] : $user->lang['TOPIC_LOCKED']);
        }
        ...
        if (!$auth->acl_get('m_edit', $forum_id))
        {
            ...
            {
                return $user->lang['CANNOT_EDIT_TIME'];
            }
            
            if ($post_data['post_edit_locked'])
            {
                return $user->lang['CANNOT_EDIT_POST_LOCKED'];
            }
        }
