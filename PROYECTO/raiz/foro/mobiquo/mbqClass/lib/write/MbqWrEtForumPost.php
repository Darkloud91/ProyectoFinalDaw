...
Class MbqWrEtForumPost extends MbqBaseWrEtForumPost {
    ...
    public function addMbqEtForumPost($oMbqEtForumPost) {
        ...
        $request->overwrite('attachment_data', $attachment_data, \phpbb\request\request_interface::POST);
    ...
    public function mDeletePost($oMbqEtForumPost, $mode, $reason) {
        ...
        $mcpMain = new mcp_main(new p_master());
    ...
    public function mApprovePost($oMbqEtForumPost, $mode) {
        if($mode == 1)
        {
            ...
            $action = 'approve';
            $request->overwrite('post_id_list', array($oMbqEtForumPost->postId->oriValue));
            $user->setup('mcp');
    ...
    public function mMovePost($oMbqEtForumPosts, $oMbqEtForum, $oMbqEtForumTopic, $topicTitle) {
        ...
        else
        {
            ...
            $request->overwrite('post_id_list', $postIds);
            $request->overwrite('forum_id', $oMbqEtForum->forumId->oriValue);
    ...
    public function reportPost($oMbqEtForumPost, $reason) {
        ...
        require_once(MBQ_APPEXTENTION_PATH . 'fake_template.' . $phpEx);
        $template = new fake_template();
        $request->overwrite('f', $oMbqEtForumPost->forumId->oriValue);
        $request->overwrite('p', $oMbqEtForumPost->postId->oriValue);
        $request->overwrite('reason_id', 4);
        $request->overwrite('report_text', $reason);
        $request->overwrite('submit', true, \phpbb\request\request_interface::POST);
        
        include_once(MBQ_APPEXTENTION_PATH . 'report_clone.' . $phpEx);
        $error = $template->getTemplateVar('ERROR');
        if(isset($error) && !empty($error))
        {
            $errors = explode('<br />', $error);
    ...
    public function mCloseReport($oMbqEtForumPost)
    {
        ...
        $request->overwrite('mode', 'reports',  \phpbb\request\request_interface::POST);
        $request->overwrite('i', 'reports',  \phpbb\request\request_interface::POST);
        $request->overwrite('action', 'close',  \phpbb\request\request_interface::POST);
        $request->overwrite('report_id_list', $report_id_list,  \phpbb\request\request_interface::POST);
