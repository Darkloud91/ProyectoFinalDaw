...
Class MbqWrEtForumTopic extends MbqBaseWrEtForumTopic {
    ...
    public function mApproveTopic($oMbqEtForumTopic, $mode) {
        if($mode == 1)
        {
            ...
            $action = 'approve';
            $request->overwrite('topic_id_list', array($oMbqEtForumTopic->topicId->oriValue));
            $user->setup('mcp');
    ...
    public function mUndeleteTopic($oMbqEtForumTopic) {
        ...
        $user->setup('mcp');
        $mcp_queue = new mcp_queue(new p_master());
        $mcp_queue->approve_topics('restore', array($oMbqEtForumTopic->topicId->oriValue), null, null);
        $error = $template->getTemplateVar('ERROR');
        if(isset($error) && !empty($error))
        {
            $errors = explode('<br />', $error);
