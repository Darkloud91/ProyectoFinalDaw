...
Class MbqWrEtUser extends MbqBaseWrEtUser {
    ...
    public function registerUser($username, $password, $email, $verified, $custom_register_fields, $profile, &$errors) {
        ...
        foreach($custom_register_fields as $key=>$value)
        {
            if(strpos($key, 'date_') === 0)
            {
                ...
                $request->overwrite(substr($key, 5) . '_year' ,$dateValues[0]);
                $request->overwrite(substr($key, 5) . '_month' ,$dateValues[1]);
                $request->overwrite(substr($key, 5) . '_day' ,$dateValues[2]);
            }
            else
            {
	...
	public function updatePassword($oldPassword, $newPassword) {
		...
		require_once($phpbb_root_path . 'includes/functions_user.' . $phpEx);
		require_once($phpbb_root_path . 'includes/functions_module.' . $phpEx);
		$request->overwrite('i', $user->data['user_id']);
		$request->overwrite('new_password', $newPassword);
		$request->overwrite('password_confirm', $newPassword);
		$request->overwrite('cur_password', $oldPassword);
		$creation_time = time() - 3600;
		$token_sid = ($user->data['user_id'] == ANONYMOUS && !empty($config['form_token_sid_guests'])) ? $user->session_id : '';
		$key = sha1($creation_time . $user->data['user_form_salt'] . 'ucp_reg_details' . $token_sid);
		$request->overwrite('creation_time', $creation_time, \phpbb\request\request_interface::POST);
		$request->overwrite('form_token', $key, \phpbb\request\request_interface::POST);
		$request->overwrite('creation_time', $creation_time, \phpbb\request\request_interface::REQUEST);
		$request->overwrite('form_token', $key, \phpbb\request\request_interface::REQUEST);
	...
	public function updateEmail($password, $email, &$resultMessage) {
		...
		require_once($phpbb_root_path . 'includes/functions_module.' . $phpEx);
		$request->overwrite('i', $user->data['user_id']);
		$request->overwrite('email', $email);
		$request->overwrite('cur_password', $password);
		$creation_time = time() - 3600;
		$token_sid = ($user->data['user_id'] == ANONYMOUS && !empty($config['form_token_sid_guests'])) ? $user->session_id : '';
		$key = sha1($creation_time . $user->data['user_form_salt'] . 'ucp_reg_details' . $token_sid);
		$request->overwrite('creation_time', $creation_time, \phpbb\request\request_interface::POST);
		...
		$request->overwrite('form_token', $key, \phpbb\request\request_interface::REQUEST);
		$request->overwrite('submit', true,\phpbb\request\request_interface::POST);
	        $request->overwrite('email_confirm', $email);
		require_once(MBQ_APPEXTENTION_PATH . 'fake_template.' . $phpEx);
		$user->setup('ucp');
		$template = new fake_template();
		$module = new p_master();
	...
	public function uploadAvatar() {
        ...
        $request->overwrite('avatar_driver', 'avatar.driver.upload');
        
        //this require apt-get install php5-gd
     
        $request->overwrite('avatar_upload_file', $_FILES['uploadfile'], \phpbb\request\request_interface::FILES);
        require_once(MBQ_APPEXTENTION_PATH . 'fake_template.' . $phpEx);
		$user->setup('ucp');
		$template = new fake_template();
		$module = new p_master();
    ...
    {                
        ...
        $request->overwrite('confirm_uid', $user->data['user_id'],  \phpbb\request\request_interface::POST);
        $request->overwrite('sess', $user->session_id,  \phpbb\request\request_interface::POST);
        $request->overwrite('confirm_key', $user->data['user_last_confirm_key'],  \phpbb\request\request_interface::POST);
                
        if($mode == 1)
        {
            $request->overwrite('add', $oMbqEtUser->userName->oriValue,  \phpbb\request\request_interface::REQUEST);
            $request->overwrite('mode', 'foes',  \phpbb\request\request_interface::REQUEST);
            $request->overwrite('submit', 1,  \phpbb\request\request_interface::POST);
        }
        else 
        {
            $request->overwrite('usernames', explode(',', $oMbqEtUser->userId->oriValue),  \phpbb\request\request_interface::REQUEST);
            $request->overwrite('submit', 'Submit',  \phpbb\request\request_interface::POST);
        }
    ...
    public function mBanUser($oMbqEtUser, $mode, $reason, $expires)
    {
        ...
        $request->overwrite('user_id', $user->data['user_id'],  \phpbb\request\request_interface::POST);
        $request->overwrite('ban', $oMbqEtUser->userName->oriValue,  \phpbb\request\request_interface::POST);
        $request->overwrite('i', 'ban',  \phpbb\request\request_interface::POST);
        $request->overwrite('bansubmit', 1,  \phpbb\request\request_interface::POST);
        if(!empty($expires)) $request->overwrite('banlength', intval(($expires - time())/60),  \phpbb\request\request_interface::POST);
        $request->overwrite('banreason', $reason,  \phpbb\request\request_interface::POST);
        ...
        if($mode == 2)
        {
            $ban_userid = TT_get_user_by_id($oMbqEtUser->userId->oriValue);
    ...
    public function mUnBanUser($oMbqEtUser)
    {
        ...
        $request->overwrite('user_id', $user->data['user_id'],  \phpbb\request\request_interface::POST);
        $request->overwrite('unban', $unban,  \phpbb\request\request_interface::REQUEST);
        $request->overwrite('i', 'ban',  \phpbb\request\request_interface::POST);
        $request->overwrite('unbansubmit', 1,  \phpbb\request\request_interface::POST);
