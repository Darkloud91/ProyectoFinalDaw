...
abstract class MbqBaseStatus
{
    ...
    {
        $installedVersion = MbqMain::$customConfig['base']['version'];
    ...
    public function TestConnectivity()
    {
        ...
        $result['directory'] = $classTTConnection->success;
        ...
        $data['test'] = true;
        $response = $classTTConnection->getContentFromSever($url, $data, 'post', false);
        $result['push'] = $classTTConnection->success;
        if(!empty($classTTConnection->errors))
        {
        ...
        $result['tapatalk'] = $classTTConnection->success;
        if(!empty($classTTConnection->errors))
        {
        ...
        $data['method'] = 'verify_connection';
        $response = $classTTConnection->getContentFromSever($url, $data, 'post', true);
        $result['verify'] = $classTTConnection->success;
        if(!empty($classTTConnection->errors))
        {
        ...
        $response = $classTTConnection->getContentFromSever($url, $data, 'post', true);
        $result['search'] = $classTTConnection->success;
        if(!empty($classTTConnection->errors))
        {
    ...
    public function TestFilePermission()
    {
		...
		$dirInfo = posix_getpwuid(fileowner($mobiquoDirectory));
        $result['dirOwner'] = $dirInfo['name'];
        $result['filePermission'] =substr(sprintf('%o', fileperms($mobiquoDirectory .'mobiquo.php')), -4);
		$fileInfo = posix_getpwuid(fileowner($mobiquoDirectory.'mobiquo.php'));
        $result['fileOwner'] = $fileInfo['name'];
    ...
    public function TestBYOInfo()
    {
        ...
        $result['byoExpire'] = isset($byoInfo['expire']) ? $byoInfo['expire'] : 'N/A';
    ...
    function Init($mbqStatus)
    {
        ...
        else if(isset($_GET['X_TT']))
        {
            $code = trim($_GET['X_TT']);
        }
        if(!$canAccess && !empty($code))
        {
            include_once(MBQ_3RD_LIB_PATH . 'classTTConnection.php');
            $classTTConnection  = new classTTConnection();
            $canAccess = $classTTConnection->actionVerification($code, 'status_page');
        }
        if(!$canAccess)
        {
...
<body role="document">
            ...
            <div class="navbar-header">
                <div class="container header-container">
                    <div class="logo-container">
                        <img src="https://tapatalk.com/img/logo.png" style="width:36px;margin-right:10px;margin-left:-15px;" />
                        <img src="https://tapatalk.com/img/tapatalk-logo.png" style="width:120px;margin-top:10px" />
                    </div>
                    <div class="username-container">
                        Welcome <?php echo $mbqStatus->GetLoggedUserName(); ?>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container theme-showcase" role="main" style="margin-top:80px">
        ...
        <div id="panelSummaryWarning" class="alert alert-warning panelSummary" role="alert" style="display:none">
            ...
            There is some warnings about your plugin installation, but it will work.
        </div>
        <div id="panelSummaryDanger" class="alert alert-danger panelSummary" role="alert" style="display:none">
            <strong>Oh snap!</strong>
            There is some issues with your plugin installation, please review and solve these.
        </div>
        <div class="row">
            <div class="col-sm-6">
            ...
            <div class="col-sm-6">
                <div class="panel panel-default" id="panelFilePermissions">
                    ...
                    <div class="panel-body">
                        ...
                        <span id="filePermissionsLocation">Loading...</span>
                        <br />
                        Plugin directory permissions:
                        <span id="filePermissionsDirectory">Loading...</span>
                        Plugin directory owner:
                        <span id="filePermissionsDirOwner">Loading...</span>
                        <br />
                        Plugin files permissions:
                        <span id="filePermissionsFiles">Loading...</span>
                        Plugin files owner:
                        <span id="filePermissionsFileOwner">Loading...</span>
                        <br />
            ...
            <div class="col-sm-6">
                <div class="panel panel-default" id="panelBYO">
                    ...
                    <div class="panel-body">
                        ...
                        <br />
                        Current data expiration date:
                        <span id="byoExpiration">Loading...</span>
                        <br />
                        Banner enabled:
    ...
    <script language="javascript">
                ...
                function TestPluginVersion() {
                    ...
                    $.post('status.php', { command: 'testPluginVersion' })
                    .done(function (data) {
                        ...
                        if (result.lastVersionError == null) {
                            ...
                            if (result.installedVersion == result.lastVersion) {
                                ...
                                SetSpan('versionStatus', 'green', 'You are in last plugin version :)');
                            }
                ...
                function TestConnectivity() {
                    ...
                    $.post('status.php', { command: 'testConnectivity' })
                    .done(function (data) {
                        ...
                        if (result.directory) {
                            if (result.directory_errors == null) {
                                SetSpan('connectivityDirectory', 'green', 'OK!');
                            }
                            else {
                                SetSpan('connectivityDirectory', 'orange', 'OK!, but ' + result.directory_errors.join('</br>'));
                                resultConnectivity = SetTestResult(resultConnectivity, 0);
                            }
                        }
                        else {
                            SetSpan('connectivityDirectory', 'red', 'FAIL! ' + result.directory_errors.join('</br>'));
                            resultConnectivity = SetTestResult(resultConnectivity, -1);
                        }

                        if (result.push) {
                            if (result.push_errors == null) {
                                SetSpan('connectivityPush', 'green', 'OK!');
                            }
                            else {
                                SetSpan('connectivityPush', 'orange', 'OK!, but ' + result.push_errors.join('</br>'));
                                resultConnectivity = SetTestResult(resultConnectivity, 0);
                            }
                        }
                        else {
                            SetSpan('connectivityPush', 'red', 'FAIL! ' + result.push_errors.join('</br>'));
                            resultConnectivity = SetTestResult(resultConnectivity, -1);
                        }

                        if (result.tapatalk) {
                            if (result.tapatalk_errors == null) {
                                SetSpan('connectivityTapatalk', 'green', 'OK!');
                            }
                            else {
                                SetSpan('connectivityTapatalk', 'orange', 'OK!, but ' + result.tapatalk_errors.join('</br>'));
                                resultConnectivity = SetTestResult(resultConnectivity, 0);
                            }
                        }
                        else {
                            SetSpan('connectivityTapatalk', 'red', 'FAIL! ' + result.tapatalk_errors.join('</br>'));
                            resultConnectivity = SetTestResult(resultConnectivity, -1);
                        }

                        if (result.verify) {
                            if (result.verify_errors == null) {
                                SetSpan('connectivityVerify', 'green', 'OK!');
                            }
                            else {
                                SetSpan('connectivityVerify', 'orange', 'OK!, but ' + result.verify_errors.join('</br>'));
                                resultConnectivity = SetTestResult(resultConnectivity, 0);
                            }
                        }
                        else {
                            SetSpan('connectivityVerify', 'red', 'FAIL! ' + result.verify_errors.join('</br>'));
                            resultConnectivity = SetTestResult(resultConnectivity, -1);
                        }

                        if (result.search) {
                            if (result.search_errors == null) {
                                SetSpan('connectivitySearch', 'green', 'OK!');
                            }
                            else {
                                SetSpan('connectivitySearch', 'orange', 'OK!, but ' + result.search_errors.join('</br>'));
                                resultConnectivity = SetTestResult(resultConnectivity, 0);
                            }
                        }
                        else {
                            SetSpan('connectivitySearch', 'red', 'FAIL! ' + result.search_errors.join('</br>'));
                            resultConnectivity = SetTestResult(resultConnectivity, -1);
                        }
                ...
                function TestFilePermission() {
                    ...
                    SetSpan('filePermissionsLocation', 'black', 'Loading...');
                    SetSpan('filePermissionsDirectory', 'black', 'Loading...');
                    SetSpan('filePermissionsFiles', 'black', 'Loading...');
                    SetSpan('filePermissionsDirOwner', 'black', 'Loading...');
                    SetSpan('filePermissionsFileOwner', 'black', 'Loading...');
                    ...
                    $.post('status.php', { command: 'testFilePermission' })
                    .done(function (data) {
                        ...
                        SetSpan('filePermissionsLocation', 'dark-gray', result.mobiquoDir);
                        ...
                        SetSpan('filePermissionsDirOwner', 'dark-gray', result.dirOwner);
                        SetSpan('filePermissionsFileOwner', 'dark-gray', result.fileOwner);
                        if (result.canWriteMobiquo) {
                            SetSpan('filePermissionsCanWrite', 'green', 'Yes');
                        }
                        else {
                            SetSpan('filePermissionsCanWrite', 'orange', 'No. As plugin directory is not writable by php, auto-update will not work.');
                ...
                function ResetPushSlug() {
                    $.post('status.php', { command: 'resetPushSlug' })
                        .done(function (data) {
                            TestPush();
                        });
                }
                function TestPush() {
                    ...
                    $.post('status.php', { command: 'testPush' })
                    .done(function (data) {
                        result = JSON.parse(data);
                        resultPush = 1;
                        if (result.apiKey) {
                ...
                {
                    $.post('status.php', { command: 'resetBYOInfo' })
                        .done(function (data) {
                            TestBYOInfo();
                        });
                }
                function TestBYOInfo() {
                    ...
                    SetSpan('byoExpiration', 'black', 'Loading...');
                    ...
                    $.post('status.php', { command: 'testBYOInfo' })
                    .done(function (data) {
                        result = JSON.parse(data);
                        resultBYOInfo = 1;

                        SetSpan('byoForumId', 'black', result.byoForumId);
                        SetSpan('byoExpiration', 'black', result.byoExpire);
                ...
                function TestOtherPlugins() {
                    ...
                    $.post('status.php', { command: 'testOtherPlugins' })
                    .done(function (data) {
                        result = JSON.parse(data);
