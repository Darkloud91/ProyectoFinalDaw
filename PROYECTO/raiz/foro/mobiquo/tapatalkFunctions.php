...
function post_html_clean($str, $returnHtml = false)
{
    ...
    return parse_bbcode($str, $returnHtml);
}
...
function tapatalk_process_bbcode($message, $uid)
{
    ...
    {
        $TT_bbcodereplace = $config['tapatalk_custom_replace'];
        if(empty($TT_bbcodereplace))
        {
            $TT_bbcodereplace = $phpbb_container->get('config_text')->get('tapatalk_custom_replace');
        }
